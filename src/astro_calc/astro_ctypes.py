...
try:
    ...
    raise ImportError(f"Cannot load Swiss Ephemeris library at {_SE_LIB}: {e}")

# ------------------------------------------------------------------
# Find the astro-calc library
_LIB_DIR = os.path.dirname(os.path.abspath(__file__))
_LIB_PATH = os.path.join(_LIB_DIR, "libastro-calc.so")
if not os.path.exists(_LIB_PATH):
    # fallback: look in parent dir (for installed package)
    _LIB_PATH = os.path.join(os.path.dirname(_LIB_DIR), "libastro-calc.so")
if not os.path.exists(_LIB_PATH):
